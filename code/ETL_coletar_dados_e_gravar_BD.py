...
def process_and_insert_chunk(data, conexao, table_name, table_schema, column_names, batch_size=20000):
    """
    ...
    try:
        ...
        for e in range(0, len(arquivos)):
            ...
            with open(extracted_file_path, 'r', encoding='latin1') as f:
                ...
                process_and_insert_chunk(data, conexao, table_name, table_schema, column_names)
...
if conexao is None:
    ...
    tabelas = {'empresa': {'schema': """CREATE TABLE empresa (cnpj_basico VARCHAR(14),razao_social VARCHAR(255),natureza_juridica VARCHAR(255),qualificacao_responsavel VARCHAR(255),capital_social DECIMAL(15, 2),porte_empresa VARCHAR(255),ente_federativo_responsavel VARCHAR(255))""", 'columns': ['cnpj_basico', 'razao_social', 'natureza_juridica', 'qualificacao_responsavel', 'capital_social', 'porte_empresa', 'ente_federativo_responsavel']},
               ...
               'moti': {'schema': """CREATE TABLE moti (cnpj_basico VARCHAR(8), cnpj_ordem VARCHAR(4), cnpj_dv VARCHAR(2), motivo_situacao_cadastral VARCHAR(2), data_situacao_cadastral VARCHAR(8))""", 'columns': ['cnpj_basico', 'cnpj_ordem', 'cnpj_dv', 'motivo_situacao_cadastral', 'data_situacao_cadastral']},
               'munic': {'schema': """CREATE TABLE munic (codigo_municipio VARCHAR(7), nome_municipio VARCHAR(50), uf VARCHAR(2))""", 'columns': ['codigo_municipio', 'nome_municipio', 'uf']},
               'natju': {'schema': """CREATE TABLE natju (codigo_natureza_juridica VARCHAR(3), descricao_natureza_juridica VARCHAR(50))""", 'columns': ['codigo_natureza_juridica', 'descricao_natureza_juridica']},   
               'pais': {'schema': """CREATE TABLE pais (codigo_pais VARCHAR(5), nome_pais VARCHAR(50))""", 'columns': ['codigo_pais', 'nome_pais']},  
               'quals': {'schema': """CREATE TABLE quals (codigo_qualificacao VARCHAR(2), descricao_qualificacao VARCHAR(50))""", 'columns': ['codigo_qualificacao', 'descricao_qualificacao']}
